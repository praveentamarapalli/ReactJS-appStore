...
class AppStore extends Component {
  state = {
    tabId: 'SOCIAL',
    selectedAppsList: appsList.filter(
      eachAppItem => eachAppItem.category === 'SOCIAL',
    ),
  }

  onClickTabItem = tabId => {
    const filteredAppsList = appsList.filter(
      eachAppItem => eachAppItem.category === tabId,
    )
    this.setState({
      tabId,
  ...
  onChangeInput = event => {
    const {tabId, selectedAppsList} = this.state
    const searchedAppsList = selectedAppsList.filter(eachAppItem =>
      eachAppItem.appName
        .toLowerCase()
    ...
    this.setState({
      tabId,
      selectedAppsList: searchedAppsList,
    })
  }
          ...
          <ul className="tab-container">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isClicked={tabId === eachTab.tabId}
                key={eachTab.tabId}
              />
            ))}
          </ul>
          <ul className="app-item-container">
            {selectedAppsList.map(eachApp => (
